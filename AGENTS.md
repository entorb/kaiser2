...
5173 is idle. Prefix the call with `--screen=NN` (the screen's two-digit number,
...
* Canvas is a 576-high design space (the Atari 320x192 at `SCALE = 3`, see
  `src/game/ui/layout.ts`) at least 960 wide, scaled with `Scale.FIT` +
  `CENTER_BOTH`. `fitCanvas()` widens `CANVAS_W` (live export, max 1380) to the
  window's landscape aspect so wide phones and monitors show no side bars;
  `GameScene.init()` re-reads the window at every screen start. Scenes are laid
  out in these **design pixels** using `frame()` / `columns()` — never hard-code
  x positions from 960, derive them from `frame()` / `CANVAS_W`.
* Mobile: a phone shows the 576 design units at ~0.68, so `FS` (theme.ts) keeps
  every font at 18 design px or more (>= 12 css px) and controls at >= 56 px.
  Words give way to icons where one exists (`ui/icon.ts`); UI strings are short
  and medieval in tone (`i18n/strings.ts`). The portrait "rotate device"
  overlay is HTML (`index.html`), filled from `t("rotate.*")` in `main.ts`.
* `VIEWPORT=844x390 OUT=tmp/phone node scripts/gen_screen_exports.mjs` exports
  every screen at phone size (the design width follows the window aspect).
...
`http://localhost:5173/kaiser2/`) and use the shared driver instead of writing a
