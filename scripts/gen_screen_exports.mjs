  ...
  return fixtureState({
    ...
    players: {
      1: {
        ...
        dom: 1,
        infant: 3,
        artell: 4, // more than one: the sabotage amount slider has a track
        kavall: 2,
        manov: 1,
        hh: 2,
      },
...
const only = args.find((a) => a.startsWith("--screen="))?.slice("--screen=".length)
...
async function capture(name) {
  await session.shot(`${OUT.replace(/^tmp\//, "")}/${name}`)
  ...
  else if (fresh.length > 0) console.warn(`⚠ ${name}: canvas-renderer pageerror (WebGL2 only)`)
  writeFileSync(`${OUT}/${name}.txt`, `# ${name}\n${formatSnapshot(await session.snapshot())}`)
  console.log(name)
}
...
const SCREENS = [
  {
    ...
    run: async () => {
      ...
      // Install button (inactive in dev; the alert opens): focus order is new
      // game, rules, language, music, share, install.
      for (let i = 0; i < 5; i++) await session.press("Tab")
  ...
  },
  {
    ...
    run: async () => {
      ...
      // Arrow next (bottom-right action button; the canvas width follows the
      // window aspect), no computers -> ruler setup.
      const designW = await page.evaluate(() => {
        const cam = window.__game.scene.getScenes(true)[0].cameras.main
        return window.__game.scale.width / cam.zoom
      })
      await click(designW - 134, 516)
      await session.wait(400)
...
if (only && !SCREENS.some(({ id }) => id === only)) {
  console.error(`unknown screen ${only}; use one of ${SCREENS.map(({ id }) => id).join(", ")}`)
  process.exit(1)
}
