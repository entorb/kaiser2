...
for (const item of (args.set ?? "").split(",").filter(Boolean)) {
  ...
  if (!profile || !(field in profile))
    throw new Error(`--set: unknown ${path}`);
  profile[field] =
    typeof profile[field] === "boolean" ? value === "true" : Number(value);
}
...
for (const item of (args.tune ?? "").split(",").filter(Boolean)) {
  ...
  if (!TUNABLES[group] || !(field in TUNABLES[group]))
    throw new Error(`--tune: unknown ${path}`);
...
function duelGame(ruleset, seed) {
  ...
  for (let guard = 0; guard < MAX_TURNS * 3; guard++) {
    ...
    if (year === SNAPSHOT && !snaps[level])
      snaps[level] = snapshot(state.players[sp]);
...
function solo(ruleset) {
  ...
  for (const level of LEVELS) {
    const games = Array.from({ length: N }, (_, i) =>
      soloGame(level, ruleset, SEED * 100003 + i),
    );
...
function duel(ruleset) {
  ...
  const games = Array.from({ length: N }, (_, i) =>
    duelGame(ruleset, SEED * 100003 + i),
  );
  for (const level of LEVELS) {
