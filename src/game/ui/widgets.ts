...
export abstract class Widget extends GameObjects.Container implements Focusable {
...
export class Panel extends GameObjects.Container {
  ...
  static decorate(scene: Scene, x: number, y: number, w: number, h: number): void {
    const g = scene.add.graphics()
    g.setPosition(x, y)
    panelFrame(g, 0, 0, w, h)
  }
...
export class Button extends Widget {
  ...
  private layoutIcon(draw: IconDraw): void {
    ...
    const size = this.text.text ? BUTTON_ICON : Math.min(this.bh - 16, BUTTON_ICON * 2)
  ...
  handleKey(event: KeyboardEvent): boolean {
    if (event.key === "Enter" || event.key === " ") {
      ...
      if (event.key === "Enter" && this.opts.onSubmit && !this.disabled) this.opts.onSubmit()
      else this.activate()
      return true
    }
    return false
...
export class ListMenu extends Widget {
    ...
    items.forEach((item, i) => {
      ...
      if (item.value !== undefined) {
        ...
        if (item.valueIcon) {
          ...
          item.valueIcon(valueIcon, valueIconX, rowY + rowH / 2, LIST_VALUE_ICON)
          this.add(valueIcon)
        }
      }
  ...
  private drawRowBackground(
    ...
    row.bg.lineStyle(highlight ? 2 : 1, highlight ? COLORS.accent : COLORS.border, 1)
    row.bg.strokeRoundedRect(inset, y + inset, this.bw - inset * 2, rowH - inset * 2, RADIUS)
  ...
  private drawRowChips(row: ListRow, y: number, rowH: number): void {
    ...
    if (row.item.icon) chips.push([SPACE.lg + this.leadW + LIST_ICON / 2, LIST_ICON])
    if (row.valueIconX !== undefined) chips.push([row.valueIconX, LIST_VALUE_ICON])
    ...
    for (const [cx, size] of chips) {
      const chip = size + 6
      row.bg.fillRoundedRect(cx - chip / 2, y + (rowH - chip) / 2, chip, chip, RADIUS)
    }
  }

  private drawRowText(row: ListRow, highlight: boolean, disabled: boolean): void {
    const accent = highlight ? COLORS.accentText : (row.item.color ?? COLORS.text)
    ...
    const accentMuted = css(!disabled && highlight ? COLORS.accentText : COLORS.muted)
...
export class StatRow extends GameObjects.Container {
  constructor(
    ...
    const value = label(scene, w, 0, opts.unit ? `${valueText} ${opts.unit}` : valueText, {
...
export class Slider extends Widget {
  ...
  constructor(
    ...
    if (opts.maxLabel) {
      const l = label(scene, bw - this.trackX, this.trackY + 24, opts.maxLabel, {
        size: FS.small,
        color: COLORS.muted,
      })
      l.setOrigin(1, 0)
    ...
    for (const tick of opts.ticks ?? []) {
      ...
      const l = label(scene, this.tickX(tick.value), this.trackY + 24, tick.label, {
        size: FS.small,
        color: COLORS.muted,
        origin: 0.5,
      })
    ...
    if (opts.cost) {
      this.costText = label(scene, bw / 2, this.trackY + 36, opts.cost(this.value), {
  ...
  private drawBefore(icon: IconDraw, text: GameObjects.Text, size: number): void {
    const left = text.x - text.width * text.originX
    icon(this.bg, left - size / 2 - 6, text.y + text.height * (0.5 - text.originY), size)
  }
  ...
  private updateTexts(): void {
    this.valueText.setText(this.opts.format?.(this.value) ?? String(this.value))
    this.valueText.setColor(css(this.opts.valueColor?.(this.value) ?? COLORS.text))
    this.infoText?.setText(this.opts.info?.(this.value) ?? "")
    if (this.infoText)
      this.infoText.setColor(css(this.opts.infoColor?.(this.value) ?? COLORS.accent))
    this.costText?.setText(this.opts.cost?.(this.value) ?? "")
    if (this.costText)
      this.costText.setColor(css(this.opts.costColor?.(this.value) ?? COLORS.accent))
  }
  ...
  protected redraw(): void {
    ...
    g.fillRoundedRect(this.trackX, this.trackY, Math.max(4, tx - this.trackX), 10, 5)
    this.drawZero()
    this.drawMarkers()
    this.drawSideIcons()
    if (this.costText?.text) this.drawBefore(drawCoinsIcon, this.costText, 14)
    if (this.opts.valueIcon) this.drawBefore(this.opts.valueIcon, this.valueText, 16)
  ...
  private drawSideIcons(): void {
    ...
    if (this.opts.minIcon)
      drawTradeIcon(this.bg, 9, this.trackY + 4, 14, this.opts.minIcon === "plus" ? 1 : -1)
...
export class SegmentedControl extends Widget {
  ...
  handleKey(event: KeyboardEvent): boolean {
    ...
    if (event.key === "ArrowLeft") {
      event.preventDefault()
      this.select((this.selected - 1 + this.segments.length) % this.segments.length)
      return true
    }
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault()
  ...
  protected redraw(): void {
    this.segments.forEach((seg, i) => {
      ...
      seg.bg.strokeRoundedRect(seg.x + 0.5, 0.5, seg.w - 1, this.bh - 1, RADIUS)
