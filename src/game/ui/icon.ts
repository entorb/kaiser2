...
export type IconDraw = (g: GameObjects.Graphics, cx: number, cy: number, size: number) => void
...
export function drawCrowdIcon(
  ...
  const bust = (x: number, y0: number, s: number, color: number) => {
    ...
    const shoulders = (pad: number) => {
      ...
      g.fillRoundedRect(x - halfW - pad, top - pad, (halfW + pad) * 2, bodyH + pad, {
        tl: r,
        tr: r,
        bl: 0,
        br: 0,
...
function wheatEar(
  ...
  for (const t of [0.42, 0.58, 0.74]) {
    ...
    for (const side of [-1, 1]) {
      g.lineBetween(px, py, px - uy * out * side + ux * up, py + ux * out * side + uy * up)
    }
  }
...
export function drawGrainIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
  wheatEar(
    g,
    cx - size * 0.1,
    cy + size * 0.5,
...
export function drawAcreIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
...
export function drawCityIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
...
export function drawCathedralIcon(
  ...
  g.fillTriangle(cx - size * 0.2, towerTop, cx + size * 0.2, towerTop, cx, cy - size * 0.48)
  g.fillStyle(COLORS.woodDark, 1)
...
export function drawArrowIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
  const s = size
  const ink = COLORS.accentText
  g.fillStyle(ink, 1)
  g.fillRect(cx - s * 0.44, cy - s * 0.05, s * 0.6, s * 0.1)
  g.fillTriangle(cx + s * 0.1, cy - s * 0.27, cx + s * 0.1, cy + s * 0.27, cx + s * 0.5, cy)
...
export function drawGearIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
  const teeth = 8
  const step = (Math.PI * 2) / teeth
  const outer = size * 0.46
  const root = size * 0.34
  const pts: PhaserMath.Vector2[] = []
  const at = (r: number, a: number) => pts.push(new V(cx + Math.cos(a) * r, cy + Math.sin(a) * r))
...
export function drawCoinsIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
  const w = size * 0.62
  const h = size * 0.26
  for (let i = 0; i < 3; i++) {
...
export function drawEventIcon(
  ...
  switch (kind) {
    ...
    case "immigrant":
      ...
      g.fillTriangle(cx + s * 0.2, cy - s * 0.28, cx + s * 0.2, cy + s * 0.28, cx + s * 0.48, cy)
      break
    case "emigrant":
      g.fillStyle(COLORS.danger, 1)
    ...
    case "market":
      g.fillStyle(COLORS.accent, 1)
      g.fillTriangle(cx - s * 0.45, cy - s * 0.05, cx + s * 0.45, cy - s * 0.05, cx, cy - s * 0.42)
      g.fillStyle(ink, 1)
    ...
    case "spy": {
      ...
      g.fillCircle(cx + s * 0.08, cy + s * 0.06, s * 0.035)
      g.fillRoundedRect(cx - s * 0.27, cy - s * 0.46, s * 0.54, s * 0.32, s * 0.09)
      g.fillStyle(COLORS.danger, 1)
      g.fillRect(cx - s * 0.27, cy - s * 0.24, s * 0.54, s * 0.07)
      g.fillStyle(ink, 1)
      g.fillRoundedRect(cx - s * 0.48, cy - s * 0.17, s * 0.96, s * 0.09, s * 0.045)
      break
    }
  }
}
...
export function drawWeatherIcon(
  ...
  const sunAt = (x: number, y: number, r: number, count: number, len: number, color: number) => {
    g.fillStyle(color, 1)
    ...
    for (let i = 0; i < count; i++) {
      ...
      g.lineTo(x + Math.cos(a) * (r + 2 + len) * u, y + Math.sin(a) * (r + 2 + len) * u)
      g.strokePath()
    }
  }
  ...
  switch (wetter) {
    ...
    case 2: {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        line(Math.cos(a) * 10, -5 + Math.sin(a) * 10, Math.cos(a) * 12, -5 + Math.sin(a) * 12)
      }
...
function speaker(g: GameObjects.Graphics, cx: number, cy: number, size: number): number {
  const s = size
  g.fillStyle(COLORS.wood, 1)
  g.fillPoints(
    [
...
export function drawSoundOffIcon(
  ...
  g.lineBetween(cx + size * 0.14, cy - size * 0.2, cx + size * 0.46, cy + size * 0.2)
  g.lineBetween(cx + size * 0.14, cy + size * 0.2, cx + size * 0.46, cy - size * 0.2)
}

/** Three linked nodes: proclaim the game to others. */
export function drawShareIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
  const s = size
  const nodes = [
...
export function drawDownloadIcon(
  ...
  g.fillTriangle(cx - s * 0.26, cy - s * 0.06, cx + s * 0.26, cy - s * 0.06, cx, cy + s * 0.22)
  g.lineStyle(Math.max(1.5, s * 0.09), COLORS.wood, 1)
  g.beginPath()
...
export function drawStockIcon(g: GameObjects.Graphics, cx: number, cy: number, size: number): void {
  const w = size * 0.84
  const h = size * 0.66
  const x = cx - w / 2
  const y = cy - h / 2
...
export function drawKontorIcon(
  ...
  const s = size
  g.fillStyle(COLORS.wood, 1)
  g.fillTriangle(cx - s * 0.5, cy - s * 0.04, cx + s * 0.5, cy - s * 0.04, cx, cy - s * 0.46)
