...
import { Button, type ListItem, ListMenu, Panel, Slider, type SliderOptions } from "./widgets"
...
const lineText = (line: AlertLine) => (typeof line === "string" ? line : line.text)
const lineIndent = (line: AlertLine) => (typeof line === "string" ? 0 : LINE_ICON + LINE_ICON_GAP)
...
function addLines(scene: Scene, panel: Panel, lines: AlertLine[], w: number): void {
  let y = 72
...
export function alert(scene: Scene, title: string, lines: AlertLine[] = []): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    const btn = new Button(scene, w - SPACE.lg - 150, h - 76, 150, 56, t("ui.ok"), {
      variant: "primary",
      onClick: finish,
    })
    m.panel.add(btn)
...
export function chooseList(
  ...
  return new Promise((resolve) => {
    ...
    m = openModal(scene, title, w, h, opts.cancel ? () => finish(-1) : undefined)
    ...
    list.bind(m.group)
    if (opts.cancel) {
      const cancel = new Button(scene, w - SPACE.lg - 150, h - 70, 150, 56, t("ui.cancel"), {
        onClick: () => finish(-1),
      })
      m.panel.add(cancel)
      cancel.bind(m.group)
...
function settle<T>(close: () => void, resolve: (value: T) => void): (value: T) => void {
  let settled = false
  return (value) => {
    if (settled) return
    settled = true
...
export type SliderPromptOptions = Omit<SliderOptions, "onChange" | "onSubmit" | "label"> & {
  title: string
}
 ...
 */
export function sliderPrompt(scene: Scene, opts: SliderPromptOptions): Promise<number> {
  const w = 560
  const h = 300
  ...
  return new Promise((resolve) => {
    ...
    const ok = new Button(scene, w - SPACE.lg - 150, h - 76, 150, 56, t("ui.ok"), {
      variant: "primary",
