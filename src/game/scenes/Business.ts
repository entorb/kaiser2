...
import { drawBuildingIcon, drawCoinsIcon, drawEventIcon, drawPointsIcon } from "../ui/icon"
...
export class Business extends GameScene {
  ...
  async create() {
    ...
    while (!done) {
      ...
      const options: ListItem[] = BUILDING_ORDER.map((kind) => {
        ...
        const required = kind === "burg" || kind === "dom" ? b.land : (p[kind] + 1) * b.land
        const cost = buildingCost(p, kind, state.rules)
        const maxed = b.max !== undefined && p[kind] >= b.max
        const count = kind === "burg" || kind === "dom" ? `${p[kind]}/${b.max}` : `${p[kind]}`
        return {
      ...
      const describe = (index: number): void => {
        ...
        if (!kind) {
          for (const icon of [costIcon, landIcon, pointsIcon]) icon.setVisible(false)
          costLabel.setText("")
          landLabel.setText("")
          pointsLabel.setText("")
          return
        ...
        const cost = buildingCost(p, kind, state.rules)
        const required = kind === "burg" || kind === "dom" ? b.land : (p[kind] + 1) * b.land
        for (const icon of [costIcon, landIcon, pointsIcon]) icon.setVisible(true)
        costLabel.setText(`${cost}`)
        costLabel.setColor(css(p.geld < cost ? COLORS.danger : COLORS.onWood))
        landLabel.setText(`${required} / ${Math.trunc(p.land)}`)
        landLabel.setColor(css(p.land < required ? COLORS.danger : COLORS.onWood))
   ...
   */
  private async buyBuilding(state: GameState, kind: BuildingKind): Promise<boolean> {
    const p = playerAt(state, state.sp)
  ...
  private async endOfTurn(state: GameState): Promise<void> {
    ...
    if (p.land + p.acker < p.leute * 10 && p.land > 0) {
      depose(p)
      await alert(this, t("business.deposedLand"), [t("business.deposedLandText")])
    }
    if (p.zoll + p.mwst + p.ein < 20) {
      depose(p)
      await alert(this, t("business.deposedTax"), [t("business.deposedTaxText")])
    }
    if (p.tod <= 0) {
      die(p)
      await alert(this, t("business.death"), [t("business.deathText"), t("business.deathHeir")])
    }
