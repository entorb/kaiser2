...
export class Highscore extends GameScene {
  ...
  async create() {
    ...
    entries.forEach((e, i) => {
      ...
      panel.add(
        label(this, SPACE.lg + rankW, y, `${titleName(e.p.titel)} ${e.p.name}`, {
          color,
          weight: "bold",
        }),
      )
      panel.add(label(this, yearCx, y, `${e.jahr}`, { mono: true }).setOrigin(0.5, 0))
      panel.add(label(this, pointsCx, y, `${e.score}`, { mono: true }).setOrigin(0.5, 0))
    })
