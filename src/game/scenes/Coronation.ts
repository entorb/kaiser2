...
const vectors = (points: number[][]) => points.map(([x, y]) => new PhaserMath.Vector2(x, y))

export function poly(g: GameObjects.Graphics, points: number[][], fill: number) {
  g.fillStyle(fill, 1)
...
function pillar(g: GameObjects.Graphics, x: number, top: number) {
  ...
  for (const dx of [8, 17, 25]) g.lineBetween(x + dx, top + 10, x + dx, FLOOR_Y - 8)
...
export class Coronation extends GameScene {
  ...
  create(data: CoronationData) {
    ...
    const banner = label(this, W / 2, BANNER_MID_Y, t("coronation.text", { name: data.name }), {
      size: 26,
      weight: "bold",
      display: true,
      color: COLORS.text,
    ...
    drawCrown(crownArt)
    this.crown = this.add.container(CROWN_X, CROWN_START_Y, [this.wings, crownArt])
    pic.add(this.crown)
  ...
  private land(pic: GameObjects.Container, banner: GameObjects.Text, flash: GameObjects.Rectangle) {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      const dot = this.add.rectangle(CROWN_X, CROWN_END_Y - 6, 4, 4, i % 2 ? GOLD_HI : PARCH)
      pic.add(dot)
      this.tweens.add({
        targets: dot,
