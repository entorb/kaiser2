...
import { drawAcreIcon, drawBuildingIcon, drawCoinsIcon, type IconDraw } from "../ui/icon"
...
import { applyLandShortage, continueAction, screenTitle, statusBar } from "./common"
...
export class Land extends GameScene {
  ...
  private async play(state: GameState): Promise<void> {
    ...
    const redraw = () => {
      ...
      this.drawLandMass(landG, SPACE.lg, 24, leftW - SPACE.lg * 2, h - 24, building, acre)
    }
    redraw()
    ...
    const cost = (v: number, good: TradeGood) => (v === 0 ? "" : moneyLabel(-spend(v, good)))
    ...
    const priceText = (good: TradeGood, v: number) => `${dealPrice(state, han, good, v)}`
  ...
  private drawLandMass(
    ...
    for (let r = 0; r < grid; r++)
      for (let c = 0; c < grid; c++) cells.push({ r, c, d: (r - mid) ** 2 + (c - mid) ** 2 })
  ...
  private applyLand(state: GameState, kind: "land" | "acker", amount: number): void {
