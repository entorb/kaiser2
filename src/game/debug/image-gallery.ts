...
export const IMAGE_SECTIONS: ImageSection[] = [
  {
    ...
    images: Array.from({ length: 7 }, (_, i) => ({
      ...
      w: PROMOTION_W,
      h: PROMOTION_H,
      draw: (g) => drawPromotionArt(g, PROMOTION_W / 2, PROMOTION_H / 2, 155, i + 1, i),
    })),
  },
  {
...
const WIDTH =
  PAD * 2 + Math.max(...IMAGE_SECTIONS.map((s) => s.perRow * cell(s).w + (s.perRow - 1) * GAP))

function galleryHeight(): number {
  return IMAGE_SECTIONS.reduce((h, s) => h + SECTION_H + rows(s) * (cell(s).h + GAP), PAD * 2)
}
  ...
  create(): void {
    ...
    for (const section of IMAGE_SECTIONS) {
      ...
      section.images.forEach((spec, i) => {
        ...
        this.card(g, section, spec, x, y + Math.floor(i / section.perRow) * (h + GAP))
      })
