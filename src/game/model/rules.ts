...
import { defaultRng, type GameState, type PlayerState, playerAt, type Rng, rand } from "./types"
...
export function harvest(state: GameState, sp: number, rng: Rng = defaultRng): HarvestResult {
 ...
 */
export function distributeGuards(target: PlayerState, rng: Rng = defaultRng): GuardLayout {
 ...
 */
function emigrants(state: GameState, p: PlayerState, u: number, rng: Rng): number {
  if (state.rules === "remake") return int((p.leute * u * (3 + rand(5, rng))) / 100)
 ...
 */
export function chronicle(state: GameState, sp: number, rng: Rng = defaultRng): ChronicleResult {
  const p = playerAt(state, sp)
  ...
  const ges = abs(int(p.leute / 42.55 + (vkorn - kaus) / 150)) + rand(5, rng) * (10 - state.wetter)

  let einw = int((kaus - vkorn) / 1300)
  einw = int((einw + rand(10, rng)) * (einw > 0 ? 1 : 0) * Math.max(0, 1 - 2 * u))
...
export function tradeHouse(state: GameState, sp: number, rng: Rng = defaultRng): TradeResult {
  const p = playerAt(state, sp)
  ...
  const gew = state.rules === "remake" ? houseProfit(state, p) : atariHouseProfit(state, p, rng)

  p.geld = p.geld - p.bd * 50
  p.geld = p.geld + gew
  let zahl = remake ? remakeTribute(p, gew) : atariDue + int((p.geld * 14) / 100)
...
export function stateIncome(state: GameState, sp: number, rng: Rng = defaultRng): number {
  const p = playerAt(state, sp)
  ...
  const se = int(((state.mg1 + state.mg2) / 100) * (p.zoll + p.ein + p.mwst) + ra)
...
export function titleAdvance(state: GameState, sp: number): boolean {
  ...
  if (p.punkte > need && p.geld > 0) {
    p.titel += 1
    if (p.titel === WIN_TITLE && p.dom === 20 && p.burg === MAX_BURG) return true
...
export function resolveSabotage(
  ...
  const sabo = saboteurs * attacker.manov + rand(6, rng) * (saboteurs > 0 ? 1 : 0)
