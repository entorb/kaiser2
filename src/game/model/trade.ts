...
export function dealPrice(
  ...
  const money =
    units > 0
      ? buyCost(state, han, good, n)
      : sellProceeds(state, han, good, n);
...
export function spareLand(p: PlayerState, kind: "land" | "acker"): number {
  ...
  const kept =
    kind === "land"
      ? Math.max(BUILDINGS.markt.land * p.markt, BUILDINGS.muhl.land * p.muhl)
      : 0;
...
export function maxSale(
  ...
  const own =
    state.rules === "remake" && good !== "grain"
      ? Math.min(holding, spareLand(p, good))
      : holding;
...
function dealRise(state: GameState, good: TradeGood, units: number): number {
  return state.rules === "remake"
    ? (MARKET.impact * units) / (2 * IMPACT_SCALE[good])
    : 0;
}
...
export function buyCost(
  ...
  return (
    flatValue(state, han, good, units) * (1 + dealRise(state, good, units))
  );
}
...
export function maxAfford(
  ...
  const b =
    state.rules === "remake"
      ? (a * MARKET.impact) / (2 * IMPACT_SCALE[good])
      : 0;
...
export function refreshEmperorStock(
  state: GameState,
  rng: Rng = defaultRng,
): void {
 ...
 */
export function updateEmperorPrices(
  state: GameState,
  trader: number,
  rng: Rng = defaultRng,
): void {
...
export function tradeGrain(
  ...
  if (amount > 0) {
    ...
    settle(
      ...
      false,
      a,
      Math.trunc(buyCost(state, han, "grain", a)),
    );
    return a;
  }
...
export function buyCheapest(
  ...
  for (let han = 0; han <= state.count; han++) {
    ...
    const units = Math.min(
      want,
      maxBuy(state, han, good),
      maxAfford(state, han, good, cash),
    );
    if (units <= 0) continue;
