...
export function taxDemotion(p: PlayerState, rules: Ruleset = "atari"): boolean {
  const overtaxed = rules === "remake" ? unrest(p) > UNREST_LIMIT : p.mwst + p.ein + p.zoll > 80
 ...
 */
export function expropriate(state: GameState, sp: number, rng: Rng = defaultRng): boolean {
  if (state.rules === "remake")
    return tributeVerdict(state.turn.abg, state.turn.zahl).seize && takeHouse(state, sp)
...
const HEIR_SUFFIXES = ["II.", "III.", "IV.", "V.", "VI.", "VII.", "VIII.", "IX.", "X."]
...
export function die(p: PlayerState, rng: Rng = defaultRng): void {
  ...
  p.titel = Math.max(0, p.titel - 1)
  p.punkte = Math.trunc(p.titel * 9 + ((p.titel + 1) / 2) * 4.5) + rand(30, rng)
